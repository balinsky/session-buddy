...
- `class.instrument` is nullable. Single value from the existing instrument list (Bb Whistle, C Whistle, Concertina, D Flute, D Generic, Fiddle, High D Whistle, Low F Whistle). For classes in a series, the UI displays `class.instrument ?? series.instrument`. A null at both levels means "no instrument focus" (a tune-only class).
- **D Generic rule.** When the resolved instrument (class or series) is `D Generic`, downstream inferences treat it as `D Flute`. Concretely: anywhere we'd auto-pre-populate per-instrument learning status from a class's instrument, a `D Generic` class contributes to the user's `D Flute` status. (`D Generic` still displays as "D Generic" in the class form and detail — it's only the inference that resolves.)
